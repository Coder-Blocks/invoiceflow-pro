...
export async function POST(req: Request) {
  try {
    ...
    const salesCost = sales.reduce(
      (sum, s) => sum + Number(s.costAmount || 0),
      0
    );
    ...
    const stockValue = stocks.reduce(
      (sum, s) => sum + Number(s.quantity || 0) * Number(s.costPrice || 0),
      0
    );

    const lowStock = stocks.filter(
      (s) => Number(s.quantity || 0) <= Number(s.lowStockThreshold || 0)
    );
...
${netProfit >= 0 ? "Good. Your business is currently profitable." : "Warning. Your business is currently running in loss."}`;
    } else if (question.includes("revenue") || question.includes("income")) {
      answer = `Your total invoice value is ${money(invoiceValue)} and collected amount is ${money(collected)}.
...
${outstanding > invoiceValue * 0.3 ? "Risk: Outstanding is high. Follow up customers immediately." : "Outstanding level looks manageable."}`;
    } else if (question.includes("expense") || question.includes("cost")) {
      answer = `Your total expenses are ${money(expenseValue)}.

${expenseValue > collected * 0.5 ? "Suggestion: Expenses are high compared to collection. Review unnecessary spending." : "Expense level looks controlled."}`;
...
${lowStock.length > 0 ? "Suggestion: Reorder low stock items soon." : "Stock level looks okay."}`;
    } else if (question.includes("expiry") || question.includes("expire")) {
      ...
      if (lowStock.length === 0) {
        ...
        answer =
          ...
          lowStock
            ...
            .map(
              (item) =>
                `• ${item.medicineName} - Qty: ${item.quantity} ${item.unitType}`
            )
            .join("\n");
      }
    } else if (question.includes("payroll") || question.includes("salary")) {
      answer = `Your payroll cost is ${money(payrollCost)}.

Generated salary slips: ${salarySlips.length}.`;
    } else {
      answer = `Here is your business summary:
...
* Sales profit: ${money(salesProfit)}
