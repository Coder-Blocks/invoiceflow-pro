            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <Button variant="outline" asChild>
                        <Link href={`/api/estimates/${params.id}/pdf`} target="_blank">
                            <Download className="mr-2 h-4 w-4" /> PDF
                        </Link>
                    </Button>
                    <Button variant="outline" asChild>
                        <Link href={`/dashboard/estimates/${params.id}/edit`}>
                    ...
                    {(estimate.status === 'SENT' || estimate.status === 'ACCEPTED') && !estimate.convertedInvoiceId && (
                        <form action={`/api/estimates/${params.id}/convert`} method="POST">
                            <Button type="submit">
