...
export default async function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const invoice = await prisma.invoice.findUnique({
        ...
        include: { customer: true, items: true, payments: true, organization: true },
    });
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <Button variant="outline" asChild><Link href={`/api/invoices/${id}/pdf`} target="_blank"><Download className="mr-2 h-4 w-4" /> PDF</Link></Button>
                    <Button variant="outline" asChild><Link href={`/dashboard/invoices/${id}/edit`}><Pencil className="mr-2 h-4 w-4" /> Edit</Link></Button>
                    {invoice.status === 'DRAFT' && <Button><Send className="mr-2 h-4 w-4" /> Mark as Sent</Button>}
                </div>
            </div>
