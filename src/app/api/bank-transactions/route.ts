...
export async function GET(req: Request) {
    ...
    const matched = searchParams.get('matched'); // 'true', 'false', or undefined
...
export async function POST(req: Request) {
    ...
    try {
        ...
        for (const row of data as any[]) {
            // Expect CSV columns: Date, Description, Amount, Reference (optional)
        ...
        // After upload, trigger auto-matching for all unmatched transactions
...
async function triggerAutoMatching(orgId: string) {
    ...
    // Get all payments for the org (that are not already matched)
    ...
    // Get all invoices (for direct invoice matching if no payment record)
    ...
    for (const txn of unmatchedTxns) {
        ...
        // Also try to match directly with invoice total if no payment
        if (matches.length === 0) {
