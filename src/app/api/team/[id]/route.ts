...
export async function PATCH(
    ...
    // Only OWNER can change roles (or ADMIN cannot change OWNER)
    ...
    if (!currentMember || currentMember.role !== 'OWNER') {
        return new NextResponse('Forbidden: Only owner can change roles', { status: 403 });
    }
    ...
    try {
        ...
        // Prevent changing OWNER role (there should always be at least one OWNER)
...
export async function DELETE(
    ...
    // Only OWNER can remove members, and members can remove themselves
    const currentMember = await prisma.organizationMember.findFirst({
        where: {
            organizationId: session.user.activeOrgId,
            userId: session.user.id,
    ...
    // Allow self-removal unless they are the last OWNER
    ...
    await prisma.auditLog.create({
        data: {
            ...
            entity: 'TEAM_MEMBER',
            entityId: targetMember.userId,
            details: { removedEmail: targetMember.user.email },
        },
    });
